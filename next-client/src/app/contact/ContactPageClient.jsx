...
import { Mail, Phone, MapPin, Clock, ExternalLink, ChevronLeft } from 'lucide-react';
...
export default function ContactPageClient() {
    ...
    return (
        <div className="contact-page-wrapper">
            ...
            <style>{`
                .contact-page-wrapper { background: var(--bg-cream); min-height: 100vh; }
                .contact-hero-alt { height: 60vh; min-height: 450px; display: flex; align-items: center; justify-content: center; position: relative; overflow: hidden; }
                .hero-bg-overlay { position: absolute; top: 0; left: 0; width: 100%; height: 100%; }
                .hero-bg-overlay::after { content: ''; position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.7)); }
                .hero-bg-overlay img { width: 100%; height: 100%; object-fit: cover; }
                .contact-main { padding: 60px 48px 120px; }
                .contact-container { max-width: 1200px; margin: 0 auto; }
                .contact-info-grid-centered { display: grid; grid-template-columns: repeat(2, 1fr); gap: 32px; }
                .contact-info-card { background: white; padding: 48px 32px; border-radius: 24px; text-align: center; box-shadow: 0 10px 40px rgba(0,0,0,0.04); border: 1px solid rgba(201, 169, 110, 0.12); transition: all 0.4s cubic-bezier(0.165, 0.84, 0.44, 1); position: relative; }
                .contact-info-card:hover { transform: translateY(-8px); border-color: var(--accent-gold); box-shadow: 0 20px 60px rgba(201, 169, 110, 0.1); }
                .location-card { cursor: pointer; }
                .card-icon { color: var(--accent-gold); margin-bottom: 24px; display: inline-flex; padding: 20px; background: var(--bg-cream); border-radius: 50%; }
                .contact-info-card h3 { font-family: var(--font-heading); font-size: 1.5rem; margin-bottom: 16px; font-weight: 500; color: var(--text-charcoal); }
                .contact-info-card p { font-size: 1rem; color: var(--text-muted); margin: 4px 0; line-height: 1.6; }
                .contact-info-card p a { color: inherit; text-decoration: none; transition: color 0.3s ease; }
                .contact-info-card p a:hover { color: var(--accent-gold); }
                .map-toggle-hint { display: inline-flex; align-items: center; margin-top: 16px; font-size: 0.8rem; font-weight: 600; text-transform: uppercase; color: var(--accent-gold); opacity: 0.8; transition: opacity 0.3s ease; }
                .mini-map-container { margin-top: 24px; border-radius: 16px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.08); border: 1px solid rgba(201, 169, 110, 0.2); animation: fadeInDown 0.4s ease-out; }
                @keyframes fadeInDown { from { opacity: 0; transform: translateY(-10px); } to { opacity: 1; transform: translateY(0); } }
                .timing-note { margin-top: 12px !important; font-style: italic; opacity: 0.8; color: var(--accent-gold) !important; font-weight: 500; }
                @media (max-width: 900px) {
                    .contact-info-grid-centered { grid-template-columns: 1fr; gap: 24px; }
                    .contact-main { padding: 40px 24px 80px; }
                }
            `}</style>
        </div>
    );
